...
def rotcurve():
    ...
    на расстоянии до 12 кпк, в 400 точках. """
    
    G = 6.67408*1.98892/(3.0856776**2)*(3.1556925**2)/3.0856776*0.1 # Перевод из СИ в кпк - Myr - 10^11 Msun 
    fkv = 3.1556925/3.0856776/1000 # Коэффициент для скорости
    theta = 0   # Тут бар ещё не вращается.
    nn = 400 
    dr = 12.0/nn
    nfi = 1000
    dfi = 2*math.pi/nfi
    ...
    def forcebar(x, y, theta):
        """ Собственно вклад бара. Бар - эллипсоид Феррера. """
        Mb = 0.13 # Масса
        ab = 4.2 # Полуоси
        cb = 1.35
    ...
    for n in range (1, nn+1, 1):
        ...
        asumhalo = 0
        asumbuldge = 0
        for j in range (0, nfi, 1): # Тут считаем средний вклад в ускорение от бара на текущем радиусе.
            fi = (j-0.5)*dfi
            x1 = n*dr*math.cos(fi)
            y1 = n*dr*math.sin(fi)
