...
G = 6.67408*1.98892/(3.0856776**2)*(3.1556925**2)/3.0856776*0.1  # Переводи в кпк - Myr - 10^11 Msun. 
fkv = 3.1556925/3.0856776/1000 # Скорость тоже.
rmax = 12.0 # Размеры галактики.
rmin = 0.02
ncol = 0 # Общее число столкновений.
dt = 0.005 # Шаг по времени.
dtcol = 0.05 # Столкновения просиходят на каждом лесятом.
acol = 0.001 # Расстояние между частицами, необходимое для столкновения.
ncf = round(dtcol/dt)
obtime = round(4.0/dt) # Время, которое частица считается ОВ - ассоциацией.
Omega = 50*fkv # Скорость вращения бара.
trot = 2*math.pi/Omega 
tgrow = 4.0*trot # Время включения бара (за 4 оборота).
Rc, Vtot, Vdisk, Vbar = rings.wwd.read_data()

def forcebar(x, y, theta, fstr, lent):
    """ Ускорение бара - с учётом его вращения -> постепенное включение в
    течение 4 оборотов. x, y - координаты текущей точки, theta - угол поворота
    бара относительно неподвижной системы, fstr - степень "включения" бара в
    начале моделирования, lent - число точек на кривой вращения. """
    Mb = 0.13 # Параметры бара.
    ab = 4.2
    cb = 1.35
    eps = math.sqrt(ab**2-cb**2)
    x1 = x*math.cos(theta)+y*math.sin(theta) # Переход в С/О, связанную с баром.
    ...
    if (fstr < 1):
        ...
        for krot in range (0, lent, 1): 
            if ((r < Rc[krot]) and (r >= Rc[krot-1])): # Считываем теоретическую скорость из кривой.
                irot = krot
        vcb = Vbar[irot-1] + (Vbar[irot] - Vbar[irot-1])/(Rc[irot] - Rc[irot-1])*(r - Rc[irot-1])
        abarx = fstr*abarx - (1 - fstr)*vcb**2/r*x1/r*fkv**2 # Уменьшаем.
...
def forcedisk(x, y, i):
    """ Подключаем из кривой вращения, чтобы не считать функции Бесселя на
    каждой итерации. i - текущий шаг по времени. """
    r = math.sqrt(x**2 + y**2)
...
def set_init(num, lent):
    """ Генерируем начальные параметры облаков в заданных пределах. num - число
    частиц, lent - точек на кривой вращения. """
...
def first_step(init_i, lent):
    """ Первый шаг моделирования. init_i - массив начальных значений. """ 
...
def main_cycle(data_arr, ii, lent, ncol):
    """ Основные вычисления. data_arr - массив данных, ii - текущий шаг по 
    времени, lent - число шагов по времени, ncol - число столкновений.  """    
    iscol = 0
    # Столкновения.
    if ((ii // ncf)*ncf == ii): # Проверяем, будут ли на этом шаге столкновения.
        iscol = 1
    if (iscol == 1):
        np.sort(data_arr, order = 'x')  # Перед столкновением сортируем массив.
    for n in range (0, len(data_arr)-1, 1): #{4}
        isend = 1
        if ((data_arr['stat'][n] == 1) or (data_arr['stat'][n] == ii)): # Не сталкиваются вылетевшие/упавшие на центр частицы и OB - ассоциации.
            isend = 0
        k = n
        while (isend == 0):
            k += 1
            if (data_arr['x'][k] - data_arr['x'][n] > acol):
                isend = 1 # Проверяем только соседние несколько частиц, тк массив сортирован по x.
            if (isend == 0 and abs(data_arr['y'][k] - data_arr['y'][n]) < acol and data_arr['stat'][k] == 1 or data_arr['stat'][k] == ii):
                ...
                data_arr['vx'][k] = vx1
                data_arr['vy'][k] = vy1
                if (ncol // 10)*10 == ncol: # Каждое 10-е столкновение превращает одно из облаков в ОВ - ассоциацию.
    ...
    for n in range (0, len (data_arr), 1): 
        if (data_arr['stat'][n] > 1) and (ii == data_arr['stat'][n] + obtime): # Возвращаем ОВ - ассоциациям, отбывшим свой срок, статус частиц.
            data_arr['stat'][n] = 1
    # Основной цикл.
    time = dt*ii
    if (time < tgrow):
